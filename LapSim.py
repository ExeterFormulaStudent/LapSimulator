...
def vehicleValues():
    ...
    for line in values:  # reading and assigning values
        try:  # error catch of the text file
            ...
            print('vehicle parameters are not numerical values.')
...
def lapfunction(dist, vel):
    '''
    This function takes an input from the user for the number of laps, with error catches to ensure it is a positive
    integer. The function runs the calculator to with the inital velocity as the last  velocity of the previous lap,
    and compiles the laps into total distance and velocity arrays, which are output.
    '''
    ...
    if lapNumber == 1:  # if only a single lap
        ...
        for n in range(0, lapNumber-1):
            ...
            calculatedArray = calculatedArray[::-1]  # reverse the array
...
if __name__ == '__main__':
    ...
    # calculator for the join function
    ...
    # acceleration from rest calculator for the join function
    ...
    # join(a[1], b[0], b[1])
    totaldistandvel = lapfunction(join(a[1], b[0], b[1])[0], join(a[1], b[0], b[1])[1])  # using join function
